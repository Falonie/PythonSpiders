    ...
    def lagou(some_url):
        try:
            ...
            for i in range(1,31):
                ...
                pattern4 = re.compile(r'<!--<i></i>-->(.*?)/')
                pattern5 = re.compile(r'<div class="industry">(\s*)(.*?)/')
                company = pattern.findall(str(bsobj))
                salary = pattern2.findall(str(bsobj))
                block = pattern3.findall(str(bsobj))
                experience = pattern4.findall(str(bsobj))[:15]
                category = pattern5.findall(str(bsobj))
                print('Page %s' % i)
                for c, s, b, e, r in zip(company, salary, block, experience, category[:15]):
                    print(c, s, b, e, r[1])
                print('\n')
